...
import guideAaAssetLibrary from '/src/assets/img/Asset-Library-Datapath.png'
import guideAaInformations from '/src/assets/img/guide-aa-autorecmenu-informations.png'
import guideAaSanctuarySource from '/src/assets/img/guide-aa-sanctuary-source.png'
import guideAaSanctuaryPrimary from '/src/assets/img/guide-aa-sanctuary-primary.png'
import guideAaSanctuarySecondary from '/src/assets/img/guide-aa-sanctuary-secondary.png'
import guideAaSanctuaryTarget from '/src/assets/img/guide-aa-sanctuary-target.png'
...
import guideAaSanctuary from '/src/assets/vid/aa-sanctuary.webm'

import seqDatabaseViewer from '/src/assets/img/seq-database-viewer.png'
import seqDatabaseListView from '/src/assets/img/seq-database-viewer-list-view.png'

import seqDatapathIcon from '/src/assets/vect/database-solid.svg'
import seqFilepathIcon from '/src/assets/vect/file-solid.svg'
import seqPlayIcon from '/src/assets/vect/play-solid.svg'
...
<Tabs syncKey="hostingMethod">
    ...
    <TabItem label="Self-Hosting">
        ...
        <Aside type="danger" title="Important">
            Before installing our Patreon module, make sure you have the space required for it. This is at least twice the size of our zip file.
        </Aside>
...
<LinkCard title="Steps for manual install" href="/jb2a-wiki/getting-started/guides#steps" />
...
<Steps>
    ...
    4. Create a new folder called **exactly** ``jb2a_patreon``. It's case-sensitive.
    ...
    7. That's it! Don't forget to enable our module in any of your worlds!
            ...
            4. Repeat the following steps depending on the number of `.zip` files you have just downloaded. In the order of publication:
                ...
                <Steps>
                    ...
                    2. Extract the contents of the `.zip` file into our module located in the FoundryVTT `data` folder. By default our module should be located in `data/modules/jb2a_patreon` and inside it should look like this:
                        ...
                        <LinkCard title="Locate the User Data Folder" href="/jb2a-wiki/getting-started/guides#foundryvtt-data-folder" />
...
Since the dropdown menus don't include all of our assets, there's a handy Custom Field in each Auto-Rec entry so you can copy and paste any datapath. To search for an animation and copy its datapath, you can use the Sequencer Database Viewer, from the Sequencer module. In the next example, we'll setup the Sanctuary spell as an On-Token effect.

<Steps>

    1. The actor called "Akra" from the previous section already has the Sanctuary spell in its list. Import another Starter Hero to be the target of the spell.

    2. In the Auto-rec, under the On-Token tab, create a new entry called Sancturay.

    3. Let's tick the Source Animation checkbox so we can add an animation on Akra, the source token, the one casting the spell. 
    
    4. Click on this button, in the Token Tools on the left-hand side toolbar to open the Sequencer Database Viewer.

            <p align="center"><Image src={seqDatabaseViewer} alt="Sequencer Database Viewer" style="width: 50%;"/></p>

            You can now browse through all our animations, search for a specific one, preview it and copy the filepath or database path, also called datapath . We recommend using the database path for more flexibility, and added intelligent behaviours from the Sequencer module. Click on the little datapath icon to copy it in your clipboard. Here, we'll search and copy this database path: ``jb2a.cast_shape.circle.single01.yellow``

            <Image src={seqPlayIcon} alt="play icon" style="display: inline; width: 30px; background-color: white; border-radius: 0.2em; padding: 0.3em;" /> Preview the animation

            <Image src={seqFilepathIcon} alt="filepath icon" style="display: inline; width: 30px; background-color: white; border-radius: 0.2em; padding: 0.3em;" /> Copy the full Filepath to the animation

            <Image src={seqDatapathIcon} alt="datapath icon" style="display: inline; width: 30px; background-color: white; border-radius: 0.2em; padding: 0.3em;" /> Copy the datapath to the animation(s)
    
    5. Alternatively, you can as easily use our <a href="https://library.jb2a.com/">Asset Library</a> online for the same results. If you click on the database path at the bottom right, it will copy it in your clipboard.

            <p align="center"><Image src={guideAaAssetLibrary} alt="Asset Library Website" style="width: 50%;"/></p>
    
    5. Paste the datapath into the custom field for the source animation.
    
            <p align="center"><Image src={guideAaSanctuarySource} alt="Sequencer Database Viewer" style="width: 50%;"/></p>

    6. By default, all the subsequent effect sections will wait for the end of this source animation section in order to start. For this example, let's set the ``Wait`` section at the bottom to be ``-2000``, which means the next section will start 2000 milliseconds before the end of this current effect section, so almost at the same time.

    7. By the way, there are many settings to explore here, and the little round blue ``i`` button will give you more information about what each setting does.

            <p align="center"><Image src={guideAaInformations} alt="Auto-Rec Informations button" style="width: 50%;"/></p>

    8. Now the Primary Animation. We'll use a boon condition for this one. From the Sequencer Database Viewer, let's copy the datapath for this animation: ``jb2a.condition.boon.01.003.yellow``. 

            <p align="center"><Image src={guideAaSanctuaryPrimary} alt="Sequencer Database Viewer" style="width: 50%;"/></p>

    9. You can add a Secondary Animation, like a particle animation. This one for example: ``jb2a.particles.inward.greenyellow.01.02``. Changing the playback rate to 2 in this case adds a little bit more dynamism to the overall feel.

            <p align="center"><Image src={guideAaSanctuarySecondary} alt="Sequencer Database Viewer" style="width: 50%;"/></p>

    10. Now, finally, the Target Animation. The animation from this section needs to stay on the target, looping, until we dismiss it. For this, we'll use one of the "complete" animations. These complete animations have extra timing information saved in the Sequencer Database, so using the datapath will let Sequencer know how to handle the intro, loop and outro sections. Let's use this datapath: ``jb2a.markers.shield_rampart.complete.03.yellow``. In the options, make sure you tick the persistent setting, and we can tick the bind visibility and bind alpha settings too, so that the animation turns invisible in case the token becomes invisible.

            <p align="center"><Image src={guideAaSanctuaryTarget} alt="Sequencer Database Viewer" style="width: 50%;"/></p>

    11. It's done! Let's target the token of an ally and cast the spell!

            <p align="center"><video  controls muted="true" ><source src={ guideAaSanctuary } type="video/webm"></source></video></p>

    12. As per the Sanctuary spell, if the ally makes an attack and we need to dismiss the spell, we can just click on the Sequencer Effect Manager and end it there as shown in the video above. 

    13. This was only one way to setup and customize a spell. There are plenty of different ways to approach this. Some prefer to use the Active Effects tab to setup persistent animations, so that the animation is dismissed automatically when the active effect is turned off. It can be beneficial if you decided to go with a fully automated game using MIDI and the like. You can also use macros and plug them into an auto-rec entry in order to go beyond what A-A's user interface has to offer. It really comes down to preference and how far you want to take things, and how much time you want to invest!

</Steps>

#### Randomisation of animations.

Using Sequencer's datapath allows for some randomisation. For example, ``jb2a.condition.boon.01.001.blue`` refers to one specific animation. If you remove the color at the end of it and only have ``jb2a.condition.boon.01.001``, Sequencer will pick one of 4 different animations, corresponding to the blue, green, red and yellow existing animations. <br></br> Furthermore, if you remove the 001 part and paste ``jb2a.condition.boon.01`` instead, Sequencer will pick at random in the 68 existing animations under that particular datapath! To better understand this, untick the "List View" option of the Sequencer Database Viewer. If you press the preview button on a category a few times, you'll see that the animation is picked at random! <br></br> In theory, you could just write ``jb2a`` and it would pick at random from **ALL** our animations but we do not advise you to try as there are too many animations to handle and it could be very costly for your computer!

                    <p align="center"><Image src={seqDatabaseListView} alt="Sequencer Database Viewer List-View" style="width: 50%;"/></p>