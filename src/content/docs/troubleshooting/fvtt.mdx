...
The dropdown menus do not include all of our assets. You can however use the "Custom" field to use a database path, referencing any of our assets. You can browse and copy the database path from the [Sequencer Database Viewer](https://fantasycomputer.works/FoundryVTT-Sequencer/#/database-basics?id=opening-the-database-viewer) or from our [Libray Asset Viewer website](https://library.jb2a.com/).
...
An animation attached to a measured template should get deleted along with the measured template. If the animation is not attached, for example if you prefer to be able to delete the template and enjoy the animation by itself, you can dismiss it from the [Sequencer Effect Manager](https://fantasycomputer.works/FoundryVTT-Sequencer/#/effect-manager?id=show-effect-manager).
If that doesn't work, it may simply be a [tile or an overhead tile (in the Foreground Layer)](https://foundryvtt.com/article/tiles/)
